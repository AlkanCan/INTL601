...
#Dependent Variable: Turnout:Main Election (whether the respondent voted or not for the main election. 
#For this variable in the absence of a presidential election in the specified year, the choice for another election is taken into account, e.g lower house)
...
#Controls:Ideology,education, polity score(democracy-autocracy),Age,Compulsory Voting, Religiosioty

#These variables are not expected to affect unemployment rate, but theoretically expected to influence voting decision. 
#Therefore they are included in the model

#Polity Score: In democracies voting matters more than it does in autocracies because it can actually change something. 
#Therefore it can be expected that in democracies citizens tend to vote more than they do in autocracies.
#Regarding this I included polity score as a control variable.
...
summary(mod)
#Based on the results, the relation between unemployment rate from a year before the election and voting turnout is positive and significant. 
#It means that people tends to vote more when unemployment rate is high a year before the election.
